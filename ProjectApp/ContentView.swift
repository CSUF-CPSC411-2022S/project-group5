    ...
    var body: some View {
        VStack {
            List(buildings, id: \.name) {
                building in
                if agent.isLookingAt(otherCoords: building.coordinates) {
                    NavigationLink(destination: NearbyBuildingInfo(description: building.description, distance: agent.coordinates.distanceTo(other: building.coordinates), bearing: agent.coordinates.bearingTo(other: building.coordinates))) {
                        Text(building.name)
                    }
                }
            }
